...
const newCourse = async (req, res) => {
    ...
    try {
        const courseInfo = {
            ...
            courseStatus: req.body.courseStatus ? req.body.courseStatus : "draft",
            owner: req.userId,
        };
...
const updateCourse = async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < course.courseInstructors.length; i++) {
            if (!req.body?.courseInstructors?.includes(course.courseInstructors[i])) {
                const instructor = await People.findById(course.courseInstructors[i]);
                if (instructor) {
        ...
        const updatedCourse = await Course.findByIdAndUpdate(courseId, courseInfo, { new: true });

        // update the courses of updatedCourse.courseInstructors

        for (let i = 0; i < updatedCourse.courseInstructors.length; i++) {
            const instructor = await People.findById(updatedCourse.courseInstructors[i]);
            if (instructor) {
...
const getCourseById = async (req, res) => {
    ...
    try {
        const course = await Course.findById(courseId).populate("courseInstructors owner enrolledStudents ratings.ratings.userId");
        if (!course) {
            return res.status(404).json({
                success: false,
                message: "No course found",
...
const getAllCourses = async (req, res) => {
    try {
        const courses = await Course.find({}).populate("courseInstructors owner enrolledStudents ratings.ratings.userId");
        if (!courses) {
            return res.status(404).json({
                success: false,
                message: "No courses found",
...
const getMyCourses = async (req, res) => {
    try {
        ...
        }).populate("courseInstructors owner enrolledStudents ratings.ratings.userId");
        if (!courses) {
            return res.status(404).json({
                success: false,
                message: "No courses found",
